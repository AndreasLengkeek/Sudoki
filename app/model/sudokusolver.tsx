...
const MODES: ModesType = {
  ...
  '9': { width: 3, height: 3, lowerSize: 17, higherSize: 50 },
};
...
export default class Sudoku {
  ...
  generate(difficulty: number): void {
    ...
    if (!this.solve()) {
        console.log('cant solve: regen')
      return this.generate(difficulty);
    }
    const r = rand(DifficultyRange[difficulty].min, DifficultyRange[difficulty].max);
    let digNumbers = gridCellSize - r;

    while (digNumbers > 0) {
      const digX = randInSizeRange();
      const digY = randInSizeRange();

      if (this.get(digX, digY) && this.allowedNumbers(digX, digY).length < maxAllowedSize) {
  ...
  defaultGrid() {
    return [...Array(this.blockSize)].map((x) => [...Array(this.blockSize).fill(0)]);
  }
  ...
  allowedNumbers(x: number, y: number) {
    ...
    if (numbersInBlock.length > 1) {
      ...
      return numbersInBlock.filter((num) => numbersInRow.includes(num) && numbersInColumn.includes(num));
    } else {
